...
public static class MockTranslationRevisionRepository
{
    ...
    public static void VerifyUpdateCalled(this Mock<ITranslationRevisionRepository> mock)
    {
        mock.Verify(x => x.Update(It.IsAny<LyricsTranslationRevisionEntity>()), Times.Once);
    }
    ...
    private static void SetupDefaults(Mock<ITranslationRevisionRepository> mock)
    {
        ...
        mock.Setup(x => x.GetByIdAsync(It.IsAny<Guid>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((LyricsTranslationRevisionEntity?)null);
