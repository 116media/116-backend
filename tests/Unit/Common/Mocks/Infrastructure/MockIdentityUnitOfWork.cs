...
public static class MockIdentityUnitOfWork
{
    ...
    /// <param name="mock">The mock instance.</param>
    /// <returns>The mock, for chaining.</returns>
    public static Mock<IIdentityUnitOfWork> SetupExecuteInTransaction<TResult>(this Mock<IIdentityUnitOfWork> mock)
    {
        mock.Setup(x =>
    ...
    public static void VerifyExecutedInTransaction<TResult>(this Mock<IIdentityUnitOfWork> mock, int times = 1)
    {
        mock.Verify(
            x =>
                x.ExecuteInTransactionAsync(
