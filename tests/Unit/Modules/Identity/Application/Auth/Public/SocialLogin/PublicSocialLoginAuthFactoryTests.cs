...
namespace _116.Unit.Tests.Modules.Identity.Application.Auth.Public.SocialLogin;
...
public class PublicSocialLoginAuthFactoryTests
{
    ...
    public async Task AuthenticateOrCreateAsync_WithValidData_ShouldReturnAuthData()
    {
        ...
        FileEntity avatarFile = new FileBuilder().Build();
    ...
    public async Task AuthenticateOrCreateAsync_WithAvatarFileReturned_ShouldUpdateUserAvatar()
    {
        ...
        UserEntity user = new UserBuilder().WithEmail(email).Build();
        FileEntity avatarFile = new FileBuilder().WithId(avatarFileId).Build();
