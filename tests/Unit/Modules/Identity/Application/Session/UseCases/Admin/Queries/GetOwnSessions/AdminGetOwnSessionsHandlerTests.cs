...
namespace _116.Unit.Tests.Modules.Identity.Application.Session.UseCases.Admin.Queries.GetOwnSessions;

/// <summary>
/// Unit tests for <see cref="AdminGetOwnSessionsHandler"/>.
/// </summary>
public class AdminGetOwnSessionsHandlerTests : BaseHandlerTest
{
    private readonly Mock<ISessionRepository> _sessionRepositoryMock;
    private readonly AdminGetOwnSessionsHandler _handler;

    public AdminGetOwnSessionsHandlerTests()
    {
        _sessionRepositoryMock = MockSessionRepository.Create();

        _handler = new AdminGetOwnSessionsHandler(_sessionRepositoryMock.Object, Mapper, TimeProvider.System);
    }
    ...
    public async Task Handle_WithSessions_ShouldReturnSessionDtos()
    {
        ...
        AdminGetOwnSessionsQuery query = new(UserId: userId, CurrentSessionId: Guid.NewGuid());
    ...
    public async Task Handle_WithActiveFilter_ShouldPassFilterToRepository()
    {
        // Arrange
        var userId = Guid.NewGuid();
        AdminGetOwnSessionsQuery query = new(UserId: userId, CurrentSessionId: Guid.NewGuid(), IsActive: true);
        _sessionRepositoryMock.SetupGetUserSessionsEmpty(userId);
    ...
    public async Task Handle_WithInactiveFilter_ShouldPassFilterToRepository()
    {
        // Arrange
        var userId = Guid.NewGuid();
        AdminGetOwnSessionsQuery query = new(UserId: userId, CurrentSessionId: Guid.NewGuid(), IsActive: false);
        _sessionRepositoryMock.SetupGetUserSessionsEmpty(userId);
