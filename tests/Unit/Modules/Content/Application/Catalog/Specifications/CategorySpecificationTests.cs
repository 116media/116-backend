...
{
    private static bool Matches(Specification<CategoryEntity> spec, CategoryEntity category) =>
    ...
    [Fact]
    public void ComposedWithContentTypeFilter_MatchesOnlyThatType()
    {
        ContentTypeEntity videoType = ContentTypeFactory.Create(nameof(EnumCoreContentType.Video));
        CategoryEntity pinned = CategoryFactory.CreatePinned(videoType);

        Matches(
                new PinnedToFeedCategorySpecification().And(new CategoryByContentTypeSpecification(videoType.Id)),
                pinned
            )
            .Should()
            .BeTrue();
        Matches(
                new PinnedToFeedCategorySpecification().And(new CategoryByContentTypeSpecification(Guid.NewGuid())),
                pinned
            )
            .Should()
            .BeFalse();
    ...
    [Fact]
    public void WithoutContentTypeFilter_MatchesAnyType()
    {
