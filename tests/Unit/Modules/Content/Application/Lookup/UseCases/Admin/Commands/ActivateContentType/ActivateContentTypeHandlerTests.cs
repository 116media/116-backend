...
/// <summary>
/// Unit tests for <see cref="ActivateContentTypeHandler"/>.
/// </summary>
public class ActivateContentTypeHandlerTests : BaseContentHandlerTest
{
    private readonly Mock<ILookupRepository> _lookupRepositoryMock;
    private readonly Mock<IContentUnitOfWork> _unitOfWorkMock;
    private readonly ActivateContentTypeHandler _handler;

    public ActivateContentTypeHandlerTests()
    {
        _lookupRepositoryMock = MockLookupRepository.Create();
        _unitOfWorkMock = MockContentUnitOfWork.Create();
        _handler = new ActivateContentTypeHandler(_lookupRepositoryMock.Object, _unitOfWorkMock.Object, Mapper);
    }
    ...
    public async Task Handle_WhenInactive_ShouldActivateAndReturnDto()
    {
        ...
        var command = new ActivateContentTypeCommand(Id: inactive.Id.ToString());

        _lookupRepositoryMock.SetupGetContentTypeByIdOrThrow(inactive);

        // Act
        ActivateContentTypeResult result = await _handler.Handle(command, CancellationToken.None);
    ...
    public async Task Handle_WhenAlreadyActive_ShouldThrowConflictException()
    {
        ...
        var command = new ActivateContentTypeCommand(Id: active.Id.ToString());
    ...
    public async Task Handle_WhenNotFound_ShouldThrowNotFoundException()
    {
        ...
        var command = new ActivateContentTypeCommand(Id: nonExistentId.ToString());
