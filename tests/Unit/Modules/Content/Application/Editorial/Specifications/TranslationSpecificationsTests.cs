...
public class TranslationSpecificationsTests
{
    ...
    [Fact]
    public void PendingTranslationRevisionSpecification_WithPendingRevision_ShouldReturnTrue()
    {
        LyricsTranslationRevisionEntity revision = LyricsTranslationRevisionFactory.Create(Guid.NewGuid());

        new PendingTranslationRevisionSpecification().IsSatisfiedBy(revision).Should().BeTrue();
    }

    [Fact]
    public void PendingTranslationRevisionSpecification_WithDecidedRevision_ShouldReturnFalse()
    {
        LyricsTranslationRevisionEntity revision = LyricsTranslationRevisionFactory.Create(Guid.NewGuid());
        revision.Accept(decidedByUserId: Guid.NewGuid());

        new PendingTranslationRevisionSpecification().IsSatisfiedBy(revision).Should().BeFalse();
    }
}
