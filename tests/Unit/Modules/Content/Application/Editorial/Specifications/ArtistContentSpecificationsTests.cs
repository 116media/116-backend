...
public class ArtistContentSpecificationsTests
{
    ...
    [Fact]
    public void HasPublishedLyrics_AloneMatchesOnlyItsOwnSource()
    {
        ArtistEntity artist = ArtistFactory.Create();
        LyricsEntity song = LyricsFactory.CreatePublishedForArtist(CategoryId, artist.Id);
        var spec = new ArtistHasPublishedLyricsSpecification(lyrics: new List<LyricsEntity> { song }.AsQueryable());

        spec.ToExpression().Compile()(artist).Should().BeTrue();
    }

    [Fact]
    public void HasPublishedVideos_AloneMatchesOnlyItsOwnSource()
    {
        ArtistEntity artist = ArtistFactory.Create();
        VideoEntity video = VideoFactory.CreatePublishedForArtist(CategoryId, artist.Id);
        var spec = new ArtistHasPublishedVideosSpecification(videos: new List<VideoEntity> { video }.AsQueryable());

        spec.ToExpression().Compile()(artist).Should().BeTrue();
    }

    [Fact]
    public void HasFullLengthRelease_IgnoresSinglesAndEps()
    {
        ArtistEntity artist = ArtistFactory.Create();
        AlbumEntity single = AlbumFactory.CreateForArtist(artist.Id, EnumReleaseType.Single);
        var spec = new ArtistHasFullLengthReleaseSpecification(albums: new List<AlbumEntity> { single }.AsQueryable());

        spec.ToExpression().Compile()(artist).Should().BeFalse();
    }

    [Fact]
    public void HasPublishedArticle_RequiresThePublishedStatus()
    {
        ArtistEntity artist = ArtistFactory.Create();
        ArticleEntity draft = ArticleFactory.Create(CategoryId);
        ArticleArtistEntity join = new ArticleArtistBuilder().WithArticle(draft).WithArtistId(artist.Id).Build();
