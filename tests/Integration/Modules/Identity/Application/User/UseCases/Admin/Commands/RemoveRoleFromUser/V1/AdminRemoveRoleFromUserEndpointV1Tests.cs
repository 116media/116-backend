...
public class AdminRemoveRoleFromUserEndpointV1Tests(PostgresFixture db) : BaseApiTest(db)
{
    ...
    public async Task AdminRemoveRole_AsSuperAdmin_Returns200()
    {
        ...
        // The revocation fact now rides the user aggregate; the security email proves it dispatched.
        ...
        (await mailerContext.OutboxEmails.Where(o => o.RecipientAddress == TestUser.AdminEmail).ToListAsync())
            .Should()
            .ContainSingle(o => o.Template == nameof(IdentityMessageTemplates.RoleChanged));
    }
