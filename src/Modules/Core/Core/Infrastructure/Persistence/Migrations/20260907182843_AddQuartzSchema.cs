...
namespace _116.Core.Infrastructure.Persistence.Migrations
{
    ...
    /// <inheritdoc />
    /// <remarks>
    /// Creates the Quartz.NET job-store tables (canonical tables_postgres.sql, scoped to the
    /// <c>quartz</c> schema) so the scheduler can run clustered against Postgres. The store is
    /// configured with <c>TablePrefix = "quartz.qrtz_"</c>.
    /// </remarks>
