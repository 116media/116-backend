...
/// <summary>
/// Handles the <see cref="AdminGetOwnSessionByIdQuery" /> to retrieve a specific session by ID.
/// </summary>
...
public class AdminGetOwnSessionByIdHandler(ISessionRepository sessionRepository, IMapper mapper, IdentityI18n i18n)
    : IQueryHandler<AdminGetOwnSessionByIdQuery, AdminGetOwnSessionByIdResult>
{
    ...
    /// <returns>A <see cref="AdminGetOwnSessionByIdResult" /> containing the session DTO.</returns>
    public async Task<AdminGetOwnSessionByIdResult> Handle(
        AdminGetOwnSessionByIdQuery query,
    ...
    )
    {
        ...
        return new AdminGetOwnSessionByIdResult(Session: sessionDto);
    }
}
