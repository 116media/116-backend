...
/// <summary>
/// Rule problems owned by the OTP purpose guard.
/// </summary>
public sealed class OtpPurposeRuleProblems : IRuleProblemCatalog
{
    /// <inheritdoc />
    public IReadOnlyDictionary<string, RuleProblem> Problems { get; } =
        new Dictionary<string, RuleProblem>
        {
            [IdentityRuleCodes.InvalidOtpPurpose] = new(
                StatusCodes.Status400BadRequest,
                nameof(BadRequestException),
                (ctx, _) => ctx.Resolve<ValidationErrorMessage>().OtpPurposeInvalid()
            ),
        };
}
