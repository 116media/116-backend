...
/// <summary>
/// Rule problems owned by the client platform guard.
/// </summary>
public sealed class ClientRuleProblems : IRuleProblemCatalog
{
    /// <inheritdoc />
    public IReadOnlyDictionary<string, RuleProblem> Problems { get; } =
        new Dictionary<string, RuleProblem>
        {
            [IdentityRuleCodes.InvalidClientPlatform] = new(
                StatusCodes.Status400BadRequest,
                nameof(BadRequestException),
                (ctx, args) => ctx.Resolve<ValidationErrorMessage>().ClientPlatformInvalid(platform: args[0])
            ),
        };
}
