...
/// </summary>
public class RolePermissionByRoleAndPermissionSpecification(Guid roleId, Guid permissionId) : Specification<RolePermissionEntity>
{
    public override Expression<Func<RolePermissionEntity, bool>> ToExpression()
    {
        var roleSpec = new RolePermissionByRoleIdSpecification(roleId);
        var permissionSpec = new RolePermissionByPermissionIdSpecification(permissionId);
        return roleSpec.And(permissionSpec).ToExpression();
    }
}
