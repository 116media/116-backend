    ...
    )
    {
        if (isRequired)
        {
            return ruleBuilder
                ...
                .NotEmpty()
                .WithMessage(msg.RoleNameRequired())
                .MaximumLength(maximumLength: RoleConstants.MaxRoleNameLength)
                .WithMessage(msg.RoleNameTooLong(RoleConstants.MaxRoleNameLength));
        }
    ...
    )
    {
        if (isRequired)
        {
            return ruleBuilder
                .Cascade(cascadeMode: CascadeMode.Stop)
                .NotEmpty()
                .WithMessage(msg.RoleDescriptionRequired())
                .MaximumLength(maximumLength: RoleConstants.MaxRoleDescriptionLength)
                .WithMessage(msg.RoleDescriptionTooLong(RoleConstants.MaxRoleDescriptionLength));
        }
