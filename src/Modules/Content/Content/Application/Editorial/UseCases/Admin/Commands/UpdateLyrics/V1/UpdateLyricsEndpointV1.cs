...
public record UpdateLyricsResponse(LyricsDto Lyrics);

/// <summary>
/// Defines the admin update lyrics endpoint.
/// Handles replacing the lyrics text of an existing lyrics page.
/// </summary>
public class UpdateLyricsEndpointV1 : ICarterModule
{
    /// <summary>
    ...
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        group
            .MapPut(
                "/{id}",
                async (string id, UpdateLyricsRequest request, IDispatcher dispatcher) =>
                {
                    var command = new UpdateLyricsCommand(Id: id, LyricsText: request.LyricsText);

                    UpdateLyricsResult result = await dispatcher.Send(request: command);

                    var response = new UpdateLyricsResponse(Lyrics: result.Lyrics);
                    return Results.Ok(response);
                }
            )
            .WithName(endpointName: UpdateLyricsMetaField.UpdateLyrics.Name)
            .WithSummary(summary: UpdateLyricsMetaField.UpdateLyrics.Summary)
            .WithDescription(description: UpdateLyricsMetaField.UpdateLyrics.Description)
            .WithAuthorization(AccountStatusPolicies.RequireActiveUser)
            .WithAuthorization(UserRolePolicies.RequireAdminOrSuperAdmin)
            .RequireRateLimiting(policyName: RateLimitPolicies.ContentBrowsing)
            .ProducesValidationProblem()
            .Produces<UpdateLyricsResponse>(statusCode: StatusCodes.Status200OK)
