...
/// <summary>
/// Handles the <see cref="DeleteShortVideoCommand" /> to permanently delete a short video
...
public class DeleteShortVideoHandler(
    IShortVideoRepository shortVideoRepository,
    ICloudinaryService cloudinaryService,
    IContentUnitOfWork unitOfWork
) : ICommandHandler<DeleteShortVideoCommand>
{
    /// <inheritdoc />
    public async Task Handle(DeleteShortVideoCommand command, CancellationToken cancellationToken)
    {
        ...
        if (shortVideo.ThumbnailStorageKey is not null)
        {
            await cloudinaryService.DeleteImageAsync(
                storageKey: shortVideo.ThumbnailStorageKey,
                cancellationToken: cancellationToken
            );
        }

        await cloudinaryService.DeleteImageAsync(
            storageKey: shortVideo.VideoStorageKey,
            cancellationToken: cancellationToken
        );
