...
public record GetAllCustomersResponse(PaginatedResult<CustomerDto> Customers);
...
/// </summary>
public class GetAllCustomersEndpointV1 : ICarterModule
{
    /// <summary>
        ...
        group
            .MapGet(
                ...
                async (IDispatcher dispatcher, int pageIndex = 0, int pageSize = 10) =>
                {
                    ...
                    var query = new GetAllCustomersQuery(PaginatedRequest: paginatedRequest);

                    GetAllCustomersResult result = await dispatcher.Send(request: query);

                    var response = new GetAllCustomersResponse(Customers: result.Customers);
                    return Results.Ok(response);
                }
            )
            .WithName(endpointName: GetAllCustomersMetaField.GetAllCustomers.Name)
            .WithSummary(summary: GetAllCustomersMetaField.GetAllCustomers.Summary)
            .WithDescription(description: GetAllCustomersMetaField.GetAllCustomers.Description)
            .WithAuthorization(AccountStatusPolicies.RequireActiveUser)
            .WithAuthorization(UserRolePolicies.RequireAdminOrSuperAdmin)
            .RequireRateLimiting(policyName: RateLimitPolicies.ContentBrowsing)
            .Produces<GetAllCustomersResponse>(statusCode: StatusCodes.Status200OK)
