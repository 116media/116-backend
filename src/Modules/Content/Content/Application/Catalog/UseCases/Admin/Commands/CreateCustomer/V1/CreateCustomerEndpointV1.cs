...
public record CreateCustomerRequest(string FullName, string Email, string? Phone, string? Company, string? Notes);
...
/// </summary>
public class CreateCustomerEndpointV1 : ICarterModule
{
    /// <summary>
    ...
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        group
            .MapPost(
                "/",
                async (CreateCustomerRequest request, IDispatcher dispatcher, HttpContext httpContext) =>
                {
                    ...
                    CreateCustomerResult result = await dispatcher.Send(request: command);

                    var response = new CreateCustomerResponse(Customer: result.Customer);
            ...
            .WithName(endpointName: CreateCustomerMetaField.CreateCustomer.Name)
            .WithSummary(summary: CreateCustomerMetaField.CreateCustomer.Summary)
            .WithDescription(description: CreateCustomerMetaField.CreateCustomer.Description)
            .WithAuthorization(AccountStatusPolicies.RequireActiveUser)
            .WithAuthorization(UserRolePolicies.RequireAdminOrSuperAdmin)
            .RequireRateLimiting(policyName: RateLimitPolicies.ContentBrowsing)
            .ProducesValidationProblem()
            .Produces<CreateCustomerResponse>(statusCode: StatusCodes.Status201Created)
